    ...
    return (
        <div className="fade-in max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3 mb-8">
                ...
                <h1 className="text-2xl font-heading font-bold text-text tracking-tight">New Session</h1>
            </div>
            ...
            <div className="flex items-center gap-2 mb-8 relative">
                ...
                {[{ n: 1, label: 'Details' }, { n: 2, label: 'Tag Topics' }].map(({ n, label }, idx) => (
                    <div key={n} className="flex-1 flex flex-col items-center gap-3">
                        ...
                        <span className={`text-xs font-semibold uppercase tracking-wider ${step >= n ? 'text-text' : 'text-text-muted/60'}`}>
                            {label}
                        </span>
                    </div>
                ))}
            ...
            {step === 1 && (
                <form onSubmit={handleDetailsSubmit} className="bg-surface border border-border p-6 md:p-8 space-y-5 fade-in rounded-xl">
                    <div>
                        <label className="block text-[11px] font-bold text-text-muted uppercase tracking-wider mb-2">Session Title *</label>
                        <input
                            required
                            value={form.title}
                            onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
                            className="w-full bg-surface-2 border border-border text-text rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition-all shadow-inner"
                            placeholder="e.g. Mock Test 1, Practice Set 3..."
                        />
                    </div>
                    <div>
                        <label className="block text-[11px] font-bold text-text-muted uppercase tracking-wider mb-2">Date</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-[11px] font-bold text-text-muted uppercase tracking-wider mb-2">Notes</label>
                        <textarea
                            value={form.notes}
                            onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
                            rows={3}
                            className="w-full bg-surface-2 border border-border text-text rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition-all resize-none shadow-inner"
                ...
                <div className="fade-in space-y-4">
                    <div className="bg-surface border border-border p-6 rounded-xl">
                        <div className="flex items-start justify-between">
                            <div>
                                <h3 className="text-lg font-heading font-semibold text-text tracking-tight mb-1">What did you cover?</h3>
                    ...
                    <div className="bg-surface border border-border p-2 md:p-4 rounded-xl max-h-[500px] overflow-y-auto">
                        {topics.length === 0 ? (
                            <div className="p-8 text-center border border-dashed border-border rounded-xl mx-2 my-2 bg-surface-3/5">
                                <GitCommit className="w-8 h-8 text-text-muted mx-auto mb-3 opacity-50" />
                                <p className="text-text-muted text-sm font-medium">No topics found in this subject.</p>
                                <p className="text-text-muted/60 text-xs mt-1">Add topics horizontally via the Subject page first.</p>
                            </div>
                        ) : (
                                ...
                                {topics.map((t) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`relative flex items-center gap-3 py-2.5 px-3 rounded-xl transition-all cursor-pointer group
                                                ${sel
                                                    ...
                                                    : 'border border-transparent hover:bg-surface-3/10'
                                                }`}
                                        ...
                                        >
                                            ...
                                            {t.depth > 0 && (
                                                <div
                                                    className="w-px h-full bg-border absolute left-0 top-0 bottom-0"
                                            ...
                                            <div style={{ marginLeft: `${t.depth * 16}px` }} className="flex items-center gap-3 flex-1">
                                                ...
                                                <div className={`w-5 h-5 rounded-md border flex-shrink-0 flex items-center justify-center transition-all duration-300
                                                    ${sel ? 'bg-primary border-primary shadow-[0_0_10px_rgba(139,92,246,0.5)]' : 'border-border bg-surface-3 group-hover:border-text-muted'}
                                                `}>
                                                    {sel && <CheckCircle2 className="w-3.5 h-3.5 text-text" strokeWidth={3} />}
                                                </div>

                                                <span className={`text-sm flex-1 transition-colors ${sel ? 'text-text font-medium' : 'text-text-muted'} ${t.depth === 0 ? 'font-heading font-semibold' : ''}`}>
                                                    {t.name}
                                                </span>
                                            </div>
