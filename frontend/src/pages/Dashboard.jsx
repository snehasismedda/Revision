        ...
        <div className="fade-in max-w-6xl mx-auto">
            ...
            <div className="flex items-end justify-between mb-8">
                <div>
                    ...
                    <h1 className="text-3xl md:text-4xl font-heading font-bold text-text tracking-tight">Dashboard</h1>
            ...
            <div className="mb-10 fade-in stagger-1">
                <div className="bg-surface border border-border-card bg-surface border border-border p-8 rounded-[2rem] border-indigo-500/20 bg-indigo-500/[0.03] relative overflow-hidden group shadow-2xl shadow-indigo-900/10">
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <h2 className="text-2xl font-heading font-bold text-text tracking-tight">AI Strategic Planner</h2>
                                    ...
                                    <p className="text-text-muted text-sm leading-relaxed max-w-xl">Intelligent analysis of your global performance with a curated roadmap for your upcoming study sessions.</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                {globalInsight && (
                                    <div className="hidden md:flex flex-col items-end mr-2">
                                        ...
                                        <div className="flex gap-1 mt-1">
                                            {[...Array(5)].map((_, i) => (
                                                <div key={i} className={`h-1 w-4 rounded-full ${i < 4 ? 'bg-indigo-500' : 'bg-surface-3/10'}`} />
                                            ))}
                                        </div>
                                    </div>
                                )}
                                ...
                                >
                                    {globalInsightLoading ? (
                                        ...
                                        <>
                                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-surface-3/10 to-transparent -translate-x-full group-hover/btn:animate-[shimmer_1.5s_infinite] pointer-events-none" />
                                ...
                                <div className="flex items-center justify-between mb-3 px-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            className={`text-[11px] font-bold flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all cursor-pointer border ${isEditingInsight ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400' : 'text-text-muted hover:text-text border-transparent hover:bg-surface-3/40'}`}
                                        >
                                            {isEditingInsight ? <><CheckCircle className="w-3 h-3" /> Save Changes</> : <><Edit2 className="w-3 h-3" /> Edit Mode</>}
                                        </button>
                                        <button
                                            onClick={handleDownloadTxt}
                                            className="text-[11px] font-bold text-text-muted hover:text-emerald-400 flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all cursor-pointer border border-transparent hover:bg-emerald-500/5 hover:border-emerald-500/10"
                                        >
                                            <Download className="w-3 h-3" /> Export (.txt)
                                        </button>
                                    </div>
                                </div>
                                <div className={`p-8 rounded-[1.5rem] bg-indigo-500/[0.02] border transition-colors duration-500 relative group/editor ${isEditingInsight ? 'border-indigo-500/40 bg-indigo-500/[0.04]' : 'border-border'}`}>
                                    {isEditingInsight ? (
                                        <textarea
                                            value={globalInsight}
                                            onChange={(e) => setGlobalInsight(e.target.value)}
                                            className="w-full bg-transparent text-text text-base leading-relaxed font-medium outline-none border-none resize-none min-h-[300px] focus:ring-0 selection:bg-indigo-500/30 font-mono scrollbar-hide"
                                            autoFocus
                                            placeholder="Craft your master study plan here..."
                                        />
                                    ) : (
                                        <div className="prose prose-invert prose-indigo max-w-none prose-p:text-text prose-p:leading-relaxed prose-headings:text-text prose-headings:font-heading prose-headings:tracking-tight prose-strong:text-indigo-400 prose-code:text-indigo-300 prose-code:bg-indigo-500/10 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded">
                                            <ReactMarkdown
                                    ...
                                    <div className="absolute top-4 right-4 opacity-0 group-hover/editor:opacity-100 transition-opacity pointer-events-none">
                                        <div className="text-[9px] px-2 py-1 rounded bg-surface-2/40 border border-border text-text-muted uppercase tracking-widest font-black">
            ...
            {weakSubjects.length > 0 && (
                <div className="mb-10 fade-in stagger-3">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-heading font-bold text-text tracking-tight">Needs Attention</h2>
                    </div>
            ...
            <div className="fade-in stagger-4 pb-12">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-heading font-bold text-text tracking-tight">Active Subjects</h2>
                    </div>
                ...
                {loading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {[...Array(3)].map((_, i) => (
                            <div key={i} className="bg-surface border border-border p-6 animate-pulse h-[140px] rounded-xl border-border" />
                        ))}
                    </div>
                ) : subjects.length === 0 ? (
                    <div className="bg-surface border border-border-panel rounded-xl p-12 text-center border-dashed border-primary/30 w-full relative overflow-hidden group">
                        ...
                        <h3 className="text-2xl font-heading font-bold text-text mb-2 tracking-tight">No subjects yet</h3>
            ...
            <div className="fade-in stagger-5 pb-12">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-heading font-bold text-text tracking-tight">Active Test Series</h2>
                    </div>
                    {testSeries.length > 6 && (
                        <button
                ...
                {loading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {[...Array(3)].map((_, i) => (
                            <div key={i} className="bg-surface border border-border p-6 animate-pulse h-[100px] rounded-xl border-border" />
                        ))}
                    </div>
                ) : testSeries.length === 0 ? (
                    <div className="bg-surface border border-border-panel p-16 rounded-xl text-center border-dashed border-border w-full relative overflow-hidden group">
                        ...
                        <h3 className="text-xl font-heading font-bold text-text mb-2 tracking-tight">No active test series</h3>
                        <p className="text-text-muted text-xs max-w-sm mx-auto mb-6 leading-relaxed">
