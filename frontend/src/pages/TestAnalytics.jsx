...
import {
    ...
    Award, Zap, BarChart2, BookOpen, Minus, AlertTriangle, Flame, Brain,
...
const ChartTip = ({ active, payload, label }) => {
    ...
    return (
        <div className="bg-[#13132a]/95 border border-white/10 rounded-xl p-3 shadow-2xl text-sm min-w-[140px]">
            <p className="text-slate-400 mb-1.5 text-xs">{payload[0]?.payload?.date || label}</p>
            {payload.map((p, i) => (
                <p key={i} style={{ color: p.color }} className="font-semibold">
                    {p.name}: {p.value}{p.name.includes('Acc') || p.name === 'Accuracy' || p.name === 'Target' ? '%' : ''}
                </p>
            ))}
        </div>
    );
...
const TestAnalytics = () => {
    ...
    return (
        <div className="flex-1 min-w-0 flex flex-col h-[100dvh] overflow-hidden bg-[#0c0c1d]">
            {/* ── Header ──────────────────────────────────────────────────── */}
            <header className="flex-shrink-0 border-b border-white/5 bg-[#0f0f1a]/90 backdrop-blur-md sticky top-0 z-20">
                <div className="px-5 py-3 flex items-center gap-3">
                    <button onClick={() => navigate(`/tests/${seriesId}`)}
                        className="w-9 h-9 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] flex items-center justify-center border border-white/8 transition-colors flex-shrink-0">
                        <ArrowLeft className="w-4 h-4 text-slate-300" />
                    </button>
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <Activity className="w-4 h-4 text-pink-400 flex-shrink-0" />
                            <h1 className="text-base font-heading font-bold text-white truncate">{testData.name} — Analytics</h1>
                        </div>
                        <p className="text-xs text-slate-500 mt-0.5">{attempts.length} attempt{attempts.length !== 1 ? 's' : ''} · avg {stats.avgAccuracy ?? 0}%</p>
                    </div>
                    <button onClick={generateInsights} disabled={aiLoading || attempts.length === 0}
                        className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-pink-600 hover:from-violet-500 hover:to-pink-500 text-white text-xs font-semibold shadow-lg shadow-pink-500/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0">
            ...
            <main className="flex-1 overflow-y-auto">
                <div className="max-w-[1100px] mx-auto p-5 md:p-7 space-y-6">
