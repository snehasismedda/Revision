                ...
                >
                    ...
                    {minimizedItems.map((item, index) => (
                        <div
                            ...
                            className="group relative flex items-center gap-2.5 p-2.5 rounded-xl bg-surface border border-border hover:border-indigo-500/30 hover:bg-surface-2 transition-all cursor-pointer shadow-lg overflow-hidden"
                            onClick={(e) => {
                                e.stopPropagation();
                        ...
                        >
                            ...
                            <div className="flex flex-col min-w-0 flex-1 pr-6">
                                <span className="text-[11px] font-bold text-text/80 tracking-tight truncate leading-tight">
                                    {item.title || 'Untitled'}
                                </span>
                            </div>
                            ...
                            <button
                                ...
                                className="absolute right-1.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-lg flex items-center justify-center bg-transparent hover:bg-rose-500/20 text-text-muted hover:text-rose-400 transition-all z-20"
                            >
            ...
            <div className="relative">
                <button
                    ...
                    className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 shadow-xl border ${isOpen
                            ...
                            : 'bg-surface border-border text-indigo-500 hover:scale-110 hover:border-indigo-500/50'
                        }`}
                >
                ...
                {/* Count Badge */}
                {!isOpen && (
                    <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-rose-600 border-2 border-surface-1 flex items-center justify-center text-[9px] font-black text-white shadow-lg animate-in zoom-in-50 duration-300">
                        {minimizedItems.length}
                    </div>
                )}
            </div>
