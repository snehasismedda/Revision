    ...
    return (
        <ModalPortal>
            ...
            <div className={`fixed inset-0 z-[110] flex flex-col overflow-hidden font-sans transition-all duration-500
                    ...
                    ${isLightMode ? 'bg-[#f8f9fa] text-text-muted' : 'bg-[#12121a] text-text'}`}>
                ...
                <div className={`flex items-center justify-between px-5 py-2.5 border-b shrink-0 backdrop-blur-md relative z-50 transition-colors duration-300
                        ${isLightMode ? 'bg-white/80 border-slate-200' : 'bg-[#12121a]/40 border-border'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {(renderType === 'doc' || renderType === 'docx') && (
                            <button
                                ...
                                className={`p-2 rounded-lg border transition-all cursor-pointer hidden lg:flex
                                        ${isLightMode ? (isSidebarOpen ? 'bg-indigo-50 border-indigo-100 text-indigo-600 hover:bg-surface-2' : 'bg-surface-2 border-slate-200 text-text-muted hover:bg-surface-2') : (isSidebarOpen ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400 hover:bg-surface-2' : 'bg-surface-2 border-border text-text-muted hover:bg-surface-2')}`}
                        ...
                        <button
                            ...
                            className={`p-2 rounded-lg border transition-all cursor-pointer mr-2
                                    ${isLightMode ? 'bg-surface-2 border-slate-200 text-text-muted hover:bg-surface-2' : 'bg-surface-2 border-border text-text-muted hover:bg-surface-2'}`}
                        >
                            {isLightMode ? <Moon size={16} /> : <Sun size={16} />}
                        </button>

                        {(onPrev || onNext) && (
                            <div className="flex items-center gap-1 mr-2 bg-black/5 dark:bg-surface-2 p-1 rounded-lg relative">
                                <button
                                    onClick={onPrev}
                                    disabled={!onPrev}
                                    className={`p-1.5 rounded-md transition-all ${onPrev ? 'cursor-pointer' : 'opacity-40 cursor-not-allowed'}
                                            ${isLightMode ? 'text-text-muted hover:bg-white shadow-sm' : 'text-text hover:bg-surface-3'}`}
                                    title="Previous File"
                                >
                                    ...
                                    <div className="relative">
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-2.5 py-1.5 transition-all rounded-lg border cursor-pointer
                                                    ...
                                                    : (isLightMode ? 'bg-white border-slate-200 text-text-muted hover:border-indigo-300' : 'bg-surface-2 border-border-hover text-text-muted hover:border-white/20')}`}
                                        >
                                        ...
                                        {isFileListOpen && (
                                                <div
                                                    ...
                                                    className={`absolute top-full right-0 mt-3 w-[340px] max-h-[520px] rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] border overflow-hidden flex flex-col z-[100] animate-in fade-in slide-in-from-top-3 duration-300 backdrop-blur-2xl
                                                            ${isLightMode ? 'bg-white/95 border-slate-200/60' : 'bg-[#13131f]/95 border-border-hover'}`}
                                                >
                                                {/* Header */}
                                                <div className={`flex items-center justify-between px-4 py-3 border-b shrink-0 ${isLightMode ? 'bg-surface-2 border-slate-200' : 'bg-surface-2 border-border'}`}>
                                                    <div className="flex items-center gap-2 text-indigo-500">
                                                        <List size={14} className="opacity-80" />
                                                        <span className="text-[11px] font-black uppercase tracking-widest">All Files</span>
                                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${isLightMode ? 'bg-surface-2 text-text-muted' : 'bg-surface-3 text-text-muted'}`}>{allFiles.length}</span>
                                                    </div>
                                                </div>

                                                {/* Search */}
                                                <div className={`px-3 py-2.5 border-b shrink-0 ${isLightMode ? 'border-slate-100' : 'border-border'}`}>
                                                    <div className="relative">
                                                        <input
                                                            autoFocus
                                                            type="text"
                                                            ...
                                                            className={`w-full text-[12px] rounded-lg px-8 py-2 outline-none border transition-all font-medium
                                                                ${isLightMode
                                                                    ? 'bg-surface-2 border-slate-200 focus:border-slate-300 text-text-muted placeholder:text-text-muted'
                                                                    : 'bg-surface-2 border-border focus:border-white/20 text-text placeholder:text-text-muted'}`}
                                                        />
                                                        <Hash className={`absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 ${isLightMode ? 'text-text-muted' : 'text-text-muted'}`} />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5 space-y-0.5 min-h-[100px]">
                                                            ...
                                                            return (
                                                                <button
                                                                    ...
                                                                    className={`w-full text-left px-3 py-3 rounded-xl flex items-center gap-4 transition-all duration-300 cursor-pointer group relative overflow-hidden
                                                                        ${isActive
                                                                            ? (isLightMode ? 'bg-indigo-50/80 shadow-sm shadow-indigo-100/50' : 'bg-indigo-500/10 border-border shadow-inner')
                                                                            : (isLightMode ? 'hover:bg-surface-2' : 'hover:bg-surface-2')}`}
                                                                >
                                                                    ...
                                                                    <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:scale-110
                                                                        ${isActive 
                                                                            ...
                                                                            : (isLightMode ? 'bg-surface-2 text-text-muted' : 'bg-surface-2 text-text-muted')}`}
                                                                    >
                                                                    ...
                                                                    <div className="min-w-0 flex-1">
                                                                        <div className={`text-[13px] font-bold truncate leading-tight transition-colors
                                                                            ${isActive ? (isLightMode ? 'text-indigo-700' : 'text-indigo-300') : (isLightMode ? 'text-text-muted' : 'text-text')}`}>
                                                    ...
                                                    {allFiles.filter(f => (f.file_name || '').toLowerCase().includes(fileListSearch.toLowerCase()) || (f.subject_name || '').toLowerCase().includes(fileListSearch.toLowerCase())).length === 0 && (
                                                        <div className="py-8 text-center">
                                                            <p className={`text-[11px] font-bold ${isLightMode ? 'text-text-muted' : 'text-text-muted'}`}>No matches found</p>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                {/* Footer Nav */}
                                                {(onPrev || onNext) && (
                                                    <div className={`flex items-center justify-between px-3 py-2.5 border-t shrink-0 ${isLightMode ? 'bg-surface-2 border-slate-200' : 'bg-surface-2 border-border'}`}>
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); onPrev?.(); }}
                                                            disabled={!onPrev}
                                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all
                                                                    ${!onPrev ? 'opacity-30 cursor-not-allowed' : (isLightMode ? 'hover:bg-surface-2 text-text-muted' : 'hover:bg-surface-3 text-text')}`}
                                                        >
                                                            <ChevronLeft size={13} strokeWidth={2.5} /> Prev
                                                        </button>
                                                        <span className={`text-[10px] font-bold tabular-nums ${isLightMode ? 'text-text-muted' : 'text-text-muted'}`}>
                                                            {fileIndex + 1} / {allFiles.length}
                                                        </span>
                                                        <button
                                                            ...
                                                            disabled={!onNext}
                                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all
                                                                    ${!onNext ? 'opacity-30 cursor-not-allowed' : (isLightMode ? 'hover:bg-surface-2 text-text-muted' : 'hover:bg-surface-3 text-text')}`}
                                                        >
                        ...
                        <div className="w-px h-5 bg-surface-2 mx-1" />
                        ...
                        <button
                            onClick={onClose}
                            className="p-1.5 bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-text rounded-lg transition-all cursor-pointer group border border-rose-500/20 hover:border-rose-500/40"
                        >
                ...
                <div className={`flex-1 relative flex items-center justify-center overflow-hidden transition-colors
                    ...
                    {loading ? (
                        ...
                        <div className="text-center p-12 max-w-sm glass-card border-rose-500/20 bg-rose-500/[0.02]">
                            ...
                            <h4 className="text-text font-bold text-lg mb-2">Error Loading Object</h4>
                            <p className="text-text-muted text-[13px] leading-relaxed mb-6">{error}</p>
                            <button onClick={() => window.location.reload()} className="px-6 py-2.5 bg-surface-2 hover:bg-surface-3 text-text text-xs font-bold uppercase tracking-widest rounded-xl transition-all border border-border-hover">Try Refresh</button>
                        </div>
                    ) : (
                        <>
                            {/* Center Viewer */}
                            <div className={`w-full h-full flex items-center justify-center ${renderType === 'image' ? 'p-0' : 'px-3 sm:px-5 lg:px-8'}`}>
                                        ...
                                        {({ zoomIn, zoomOut, resetTransform, state }) => (
                                            <div className="relative w-full h-full flex flex-col">
                                                ...
                                                <div className={`absolute left-1/2 -translate-x-1/2 bottom-8 z-50 flex items-center gap-1.5 p-1.5 rounded-2xl backdrop-blur-xl border shadow-xl transition-colors duration-300
                                                    ...
                                                    <button onClick={() => zoomOut()} className={`p-2 rounded-xl transition-all cursor-pointer ${isLightMode ? 'hover:bg-surface-2 text-text-muted' : 'hover:bg-surface-3 text-text'}`} title="Zoom Out"><ZoomOut size={18} strokeWidth={2} /></button>
                                                    <button onClick={() => zoomIn()} className={`p-2 rounded-xl transition-all cursor-pointer ${isLightMode ? 'hover:bg-surface-2 text-text-muted' : 'hover:bg-surface-3 text-text'}`} title="Zoom In"><ZoomIn size={18} strokeWidth={2} /></button>
                                                    <button onClick={() => resetTransform()} className={`p-2 rounded-xl transition-all cursor-pointer ${isLightMode ? 'hover:bg-surface-2 text-text-muted' : 'hover:bg-surface-3 text-text'}`} title="Reset Zoom"><RefreshCw size={18} strokeWidth={2} /></button>
                                                </div>
                                ...
                                {renderType === 'xlsx' && (
                                    <div className={`w-full h-full flex flex-col overflow-hidden relative rounded-xl border shadow-lg transition-colors duration-300
                                            ${isLightMode ? 'bg-white border-slate-300/80 shadow-slate-200/50' : 'bg-[#1a1a24] border-border shadow-black/50'}`}>
                                        <div className="flex-1 overflow-auto custom-scrollbar p-0">
                                            <table className="w-full border-collapse">
                                                <thead className="sticky top-0 z-10">
                                                    {excelData && excelData.length > 0 && (
                                                        <tr className={`border-b transition-colors duration-300 ${isLightMode ? 'bg-surface-2 border-slate-300' : 'bg-[#2a2a35] border-white/20'}`}>
                                                            <th className={`px-3 py-2 text-center w-12 text-xs font-semibold border-r sticky left-0 z-20 transition-colors
                                                                    ${isLightMode ? 'bg-surface-2 text-text-muted border-slate-300' : 'bg-[#2a2a35] text-text-muted border-white/20'}`}></th>
                                                            {excelData[0].map((cell, ci) => (
                                                                <th key={ci} className={`px-4 py-2 text-left text-xs font-semibold border-r transition-colors
                                                                        ${isLightMode ? 'text-text-muted border-slate-300' : 'text-text border-white/20'}`}>
                                                ...
                                                <tbody>
                                                    {excelData && excelData.slice(1).map((row, ri) => (
                                                        <tr key={ri} className={`border-b transition-colors
                                                                ${isLightMode ? 'border-slate-200 bg-white' : 'border-border-hover bg-[#1a1a24]'}`}>
                                                            <td className={`px-3 py-2 text-center text-xs font-medium border-r sticky left-0 z-10 transition-colors
                                                                    ${isLightMode ? 'bg-surface-2 text-text-muted border-slate-200' : 'bg-[#21212b] text-text-muted border-border-hover'}`}>{ri + 1}</td>
                                                            {row.map((cell, ci) => (
                                                                <td key={ci} className={`px-4 py-2 text-sm border-r whitespace-nowrap transition-colors
                                                                        ${isLightMode ? 'text-text-muted border-slate-200' : 'text-text border-border-hover'}`}>
                                        ...
                                        {excelSheets.length > 0 && (
                                            <div className={`flex items-center gap-1 overflow-x-auto px-2 border-t shrink-0 transition-colors duration-300
                                                    ${isLightMode ? 'bg-surface-2 border-slate-200' : 'bg-[#2a2a35] border-border-hover'}`}>
                                                {excelSheets.map((name, i) => (
                                                    <button
                                                        ...
                                                        className={`px-4 py-2 text-xs font-medium whitespace-nowrap border-b-2 transition-all cursor-pointer
                                                                ...
                                                                : (isLightMode ? 'border-transparent text-text-muted hover:bg-surface-2' : 'border-transparent text-text-muted hover:bg-surface-2')}`}
                                                    >
                                                        {name}
                                                    </button>
                                                ))}
                                ...
                                {(renderType === 'doc' || renderType === 'docx') && (
                                    <div className={`w-full h-full flex overflow-hidden rounded-xl border shadow-lg transition-colors duration-300
                                            ${isLightMode ? 'bg-white border-slate-300/80 shadow-slate-200/50' : 'bg-[#111116] border-border shadow-black/50'}`}>
                                        ...
                                        <div className={`hidden lg:flex flex-col border-r shrink-0 overflow-y-auto custom-scrollbar transition-all duration-300
                                                ...
                                                ${isLightMode ? 'bg-surface-2 border-slate-200' : 'bg-[#21212b] border-border-hover'}`}>
                                            ...
                                            <div className="flex-1 py-4 px-2 space-y-0.5">
                                                {(() => {
                                                    ...
                                                    if (headers.length === 0) {
                                                        return <div className={`p-4 text-center text-xs ${isLightMode ? 'text-text-muted' : 'text-text-muted'}`}>No outline entries</div>;
                                                    }
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <span className={`block truncate text-xs transition-colors
                                                                        ${level === 1 ? `font-semibold ${isLightMode ? 'text-text-muted group-hover:text-black' : 'text-text group-hover:text-text'}` : `font-medium ${isLightMode ? 'text-text-muted' : 'text-text-muted'}`}`}>
                                                                    {h.textContent}
                                                                </span>
                                                            </button>
                                                        );
