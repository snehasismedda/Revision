...
import { X, PlusCircle, Trash2, Save, FileText, Image as ImageIcon, Camera, RefreshCcw, Check, RotateCw, Scissors } from 'lucide-react';
import ModalPortal from '../ModalPortal.jsx';
import ImageCropper from '../common/ImageCropper.jsx';

const AddSolutionModal = ({ isOpen, onClose, subjectId, onSolutionAdded, questionId, initialTitle, initialContent }) => {
    ...
    const handleAddSolution = async (e) => {
        e.preventDefault();
        // Title and content are now non-mandatory, but we should have at least something (title, content, or image)
        if (!title.trim() && !content.trim() && !solutionImage) {
            return toast.error('Please provide either a title, content, or an image solution');
        }
        
        setAddingSolution(true);
        try {
            const { solution } = await solutionsApi.create(subjectId, {
                title: title.trim() || '',
                content: content.trim() || '',
                questionId,
                sourceImageContent: mainType === 'image' ? solutionImage : null,
            });
            onSolutionAdded(solution);
            handleModalClose();
                ...
                >
                    <div className="flex items-center justify-between px-7 py-5 border-b border-border shrink-0">
                        <h3 className="text-lg font-heading font-semibold text-text flex items-center gap-3">
                            ...
                            {isCropping ? 'Crop Solution Image' : 'Add New Solution'}
                        </h3>
                        ...
                        <>
                                    ...
                                    <div className="space-y-4 mb-6">
                                        ...
                                        {imageMethod === 'camera' ? (
                                            ...
                                            <div className="relative border-2 border-dashed border-border rounded-xl p-8 flex flex-col items-center justify-center bg-surface-2 hover:bg-surface-3 transition-all cursor-pointer min-h-[200px]">
                                                ...
                                                {solutionImage ? <img src={solutionImage} alt="Preview" className="max-h-[300px] object-contain rounded-lg shadow-lg mb-4" /> : <div className="text-center"><ImageIcon className="w-10 h-10 text-text-muted/40 mx-auto mb-3" /><h4 className="text-text font-semibold mb-1 text-[13px]">Upload your solution image</h4><p className="text-[11px] text-text-muted">Drag or click to choose a file</p></div>}
                                            </div>
                                        )}
                                    </div>
                                )}

                                <form id="add-solution-form" onSubmit={handleAddSolution} className="space-y-5 flex flex-col">
                                    <div>
                                        <label className="block text-[10px] font-extrabold text-text-muted uppercase tracking-[0.18em] mb-2.5">Solution Title (Optional)</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-extrabold text-text-muted uppercase tracking-[0.18em] mb-2.5">Solution Content (Markdown) (Optional)</label>
                            ...
                            <div className="px-7 py-5 border-t border-border shrink-0 flex items-center justify-end bg-surface-3">
                                <div className="flex gap-3">
                                    ...
                                    <button form="add-solution-form" type="submit" disabled={addingSolution || (!title.trim() && !content.trim() && !solutionImage)} className="bg-primary text-white shadow-lg text-[13px] font-bold px-8 py-3 rounded-xl disabled:opacity-50 transition-all flex items-center gap-2 cursor-pointer active:scale-95">
                                        {addingSolution ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Save className="w-4 h-4" /> Save Solution</>}
                                    </button>
                                </div>
                            </div>
                        </>
