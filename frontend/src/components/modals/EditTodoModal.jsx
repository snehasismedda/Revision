...
const EditTodoModal = ({ isOpen, onClose, onSubmit, todo, groups = [] }) => {
    ...
    useEffect(() => {
        if (isOpen && todo) {
            ...
            setSubTodos(todo.subTodos ? [...todo.subTodos] : []);
            setCurrentSubInput('');
        }
    }, [isOpen, todo]);
    ...
    const handleAddSubTask = (e) => {
        ...
        setSubTodos(prev => [...prev, { title: currentSubInput.trim(), status: 'pending', id: `temp_${Date.now()}` }]);
        setCurrentSubInput('');
    };
    ...
    const handleSubmit = (e) => {
        ...
        onSubmit({
            ...
            subTodos: subTodos.map(s => ({
                id: s.id && !String(s.id).startsWith('temp_') ? s.id : undefined,
                title: s.title,
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop fade-in" onClick={onClose}>
                ...
                >
                    ...
                    <div className="px-7 py-6 overflow-y-auto custom-scrollbar">
                        <form id="edit-todo-form" onSubmit={handleSubmit} className="space-y-5">
                            {/* Title */}
                            <div>
                                ...
                                <div className="space-y-2 mb-2">
                                    {subTodos.map((sub, idx) => (
                                        <div key={sub.id || idx} className="flex items-center gap-2 bg-surface-2/80 px-3 py-2 rounded-xl border border-border">
                                            <span className={`w-1.5 h-1.5 rounded-full ${sub.status === 'completed' ? 'bg-emerald-400' : 'bg-primary/80'}`} />
                                            <span className={`flex-1 text-[13px] font-medium ${sub.status === 'completed' ? 'line-through text-text-muted' : 'text-text'}`}>
                                                {sub.title}
                                            </span>
                                            <button
                                                type="button"
